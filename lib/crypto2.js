...
var createPassword = function (callback) {
  // As the 'hex' encoding uses two digits for each byte, for a password with
  // 32 bytes length, only 16 bytes need to be read.
...
var sha256Sign = function (text, privateKey) {
  return privateKey.hashAndSign('sha256', text, undefined, 'hex');
};
